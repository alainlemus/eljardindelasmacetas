                ...
                <a href="{{ route('catalog.product', $product->slug) }}" class="flex-shrink-0 w-36">
                    <div class="bg-white rounded-xl overflow-hidden shadow-md">
                        ...
                        <div class="p-2">
                            ...
                            <p class="text-primary font-bold text-sm">${{ number_format($product->price, 2) }}</p>
                        </div>
                    </div>
                </a>
                @endforeach
    ...
    <section class="py-4 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            <p class="text-gray-500 text-sm mb-3">
                ...
                <a href="{{ route('catalog', ['category' => request()->get('category')]) }}" class="text-primary ml-2">Limpiar</a>
            </p>
            @endif
                ...
                <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                    <a href="{{ route('catalog.product', $product->slug) }}">
                        <div class="aspect-square bg-gray-100 relative overflow-hidden">
                            ...
                            <span class="absolute top-1.5 left-1.5 bg-accent text-white text-xs px-1.5 py-0.5 rounded font-medium">
                                ★
                            </span>
                            @endif
                            @if($product->stock <= 3 && $product->stock > 0)
                            <span class="absolute top-1.5 right-1.5 bg-orange-500 text-white text-xs px-1.5 py-0.5 rounded font-medium">
                                ¡Últimos!
                            </span>
                            @endif
                            @if($product->stock == 0)
                            <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                                <span class="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">Agotado</span>
                            </div>
                            @endif
                        </div>
                        <div class="p-2.5">
                            @if($product->category)
                            <p class="text-xs text-secondary font-medium mb-0.5">{{ $product->category->name }}</p>
                            @endif
                            <h3 class="font-semibold text-dark text-sm line-clamp-2 mb-1">{{ $product->name }}</h3>
                            <p class="text-xs text-gray-400 mb-1.5">SKU: {{ $product->sku }}</p>
                            <div class="flex items-center justify-between">
                                <span class="text-primary font-bold text-lg">${{ number_format($product->price, 2) }}</span>
                                <span class="text-xs {{ $product->stock > 5 ? 'text-green-600' : ($product->stock > 0 ? 'text-orange-500' : 'text-red-500') }}">
            ...
            <div class="text-center py-12">
                ...
                <a href="{{ route('catalog') }}" class="inline-block mt-4 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium">
